...
public class SchedulingTest {
    ...
    @Autowired
    private TopNService csdnTopNService;
    ...
    //总体概况
    public Map<String, Object> doGetTopNUrl()throws Exception{
    ...
    void doGetCsdnTopNVoteUrl() throws Exception {
        ...
        List<Vote> csdnTopNList = new ArrayList<>();
        //构建当次爬取的时间
        Date date = new Date();
        //对获取到的tr节点集合进行遍历
        for (int i = 0; i < trs.size(); i++) {
            ...
            vote.setRanking(tds.get(0).text().substring(1,tds.get(0).text().length()-1));
            //获取博主名字
            vote.setName(tds.get(1).text());
            //获取实时票数并将票字去掉，只保留票数的数字
            vote.setNowVotes(tds.get(2).text().substring(0,tds.get(2).text().length()-1));
            vote.setCreateDate(date);
            //将博主的投票数据添加到集合中
            csdnTopNList.add(vote);
        }
